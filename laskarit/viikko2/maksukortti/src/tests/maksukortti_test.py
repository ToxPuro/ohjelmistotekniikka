...
class TestMaksukortti(unittest.TestCase):
    ...
    def test_negatiivisen_maaran_lataaminen_ei_muuta_saldoa(self):
        self.kortti.lataa_rahaa(-10)
        self.assertEqual(str(self.kortti), "Kortilla on rahaa 10 euroa")

    def test_pystyy_maksamaan_tasan_edullinen(self):
        kortti = Maksukortti(2.5)
        kortti.syo_edullisesti()
        self.assertEqual(str(kortti), "Kortilla on rahaa 0.0 euroa")

    def test_pystyy_maksamaan_tasan_maukkaan(self):
